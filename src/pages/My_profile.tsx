  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (profile?.petName) setName(profile.petName);
        if (profile?.dogBreed?.name) setBreed(profile.dogBreed.name);
        if (profile?.petBirthDate) setBirth(profile.petBirthDate);
        if (profile?.petSize) {
